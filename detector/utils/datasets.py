...
img_formats = ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.ppm']
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=416, half=False):
        ...
        if os.path.isdir(path):
            ...
            # files = [file for file in files if (not ('-' in file)) and (os.path.isfile(file))]
            '''
            files = []
            fds = glob.glob('%s/*'%path)
            for fd in fds:
                fdfiles = sorted(glob.glob(os.path.join(fd, '*.*')))
                files.extend(fdfiles)
            '''
    ...
    def __next__(self):
        ...
        # Padded resize
        ims = []
        for sz in self.img_size:
            img, *_ = letterbox(img0, new_shape=sz)

    ...
    for image_i, pred in enumerate(prediction):
        ...
        for c in pred[:, -1].unique():
            ...
            if n == 1:
                ...
                dc = dc[:20]  # limit to first 100 boxes: https://github.com/ultralytics/yolov3/issues/117
...
def non_max_suppression_multires(prediction, res_nums, res_shapes, conf_thres=0.5, nms_thres=0.5):
    """
    Removes detections with lower object confidence score than 'conf_thres'
    Non-Maximum Suppression to further filter detections.
    Returns detections with shape:
    ...
    for image_i, pred in enumerate(prediction):
        ...
        pred_all = pred
        preds = []
        for resi, num in enumerate(res_nums):
            pred = pred_all[be:be+num]
            be += num
            ...
            pred_newres = scale_coords(res_shapes[resi][0], pred[:, :4], res_shapes[resi][1]).round()
            pred[:, :4] = pred_newres
        ...
        for c in pred[:, -1].unique():
            ...
            if n == 1:
                det_max.append(dc)  # No NMS required if only 1 prediction
                continue
            elif n > 20 * len(res_nums):
                dc = dc[:20 * len(res_nums)]  # limit to first 100 boxes: https://github.com/ultralytics/yolov3/issues/117
...
def plot_images(imgs, targets, paths=None, fname='images.jpg'):
    ...
    bs, c, h, w = imgs.shape  # batch size, _, height, width
    ...
    for i in range(bs):
        ...
        if c == 3:
            ...
            plt.subplot(ns, ns, i + 1).imshow(imgs[i, 0])

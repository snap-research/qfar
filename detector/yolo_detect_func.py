# Doesn't work, don't use this one
import argparse
...
# Sizhuo: make this callable from other Python scripts
def detect_iter(args):
    detector_path = os.path.dirname(__file__)
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', type=str, default=detector_path+'/cfg/yolov3_qr.cfg', help='cfg file path')
    parser.add_argument('--data', type=str, default=detector_path+'/data/coco_qr.data', help='coco.data file path')
...
#     parser.add_argument('--output', type=str, default='/u6/a/wenzheng/remote2/Snap_project/QRrealcap/QR-code-video-data/rotation/rotation_res1280x720_focus@1m-detectaaaaa', help='output folder')  # output folder
    ...
    parser.add_argument('--weights', type=str, default=detector_path+'/weights_qr2/best_large_code.pt', help='path to weights file')
    parser.add_argument('--source', type=str, default=detector_path+'/inference_input', help='source')  # input file/folder, 0 for webcam
...
#    parser.add_argument('--img-size', type=int, default=[2560, 1280, 640], help='inference size (pixels)')
    ...
    opt.view_img = False
    print(opt)
    # Sizhuo: redefine input/output dir, better interfacing with other scripts
    input_dir = opt.source + '/data'
    input_intm_dir = opt.source + '/inference_input_intermediate'
    output_intm_dir = opt.source + '/inference_output_intermediate'
    output_dir = opt.source + '/inference_output'
    ...
    with torch.no_grad():
        if False:
            ...
            if os.path.exists(input_intm_dir):
                shutil.rmtree(input_intm_dir)
            os.makedirs(input_intm_dir)
            if os.path.exists(output_dir):
                shutil.rmtree(output_dir)
            os.makedirs(output_dir)
            if os.path.exists(output_intm_dir):
                shutil.rmtree(output_intm_dir)
            os.makedirs(output_intm_dir)
            ...
            while k_round < 1:
                if k_round == 0:
                    ...
                    opt.source = input_dir
                    opt.output = output_intm_dir
                    ...
                    filenames = glob.glob(input_intm_dir + '/*.*')
                    opt.source = input_intm_dir
                    opt.output = output_intm_dir
                ...
                for filename in filenames:
                    im = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
                    if opt.source == input_intm_dir:
                        os.remove(filename)

                    figurename = os.path.basename(filename)

                    try:
                        f = open(output_intm_dir + '/' + figurename + '.txt', "r")
                    except:
                        continue
                    detectionResults = []
                    ...
                    for loc_code in detectionResults:
                        x0 = detectionResults[cnt][0].astype(np.int)
                        y0 = detectionResults[cnt][1].astype(np.int)
                        x1 = detectionResults[cnt][2].astype(np.int)
                        y1 = detectionResults[cnt][3].astype(np.int)
                        if len(im.shape) == 3:
                            print('3 channels')
                            im[y0:y1, x0:x1, :] = 0
                        else:
                    ...
                    cv2.imwrite(input_intm_dir + '/' + figurename, im)

                k_round += 1
    
    for filename in glob.glob(output_intm_dir+ '/*'):
        if os.path.isdir(filename):
            shutil.copytree(filename, output_dir)
        else:
